...
void update_long_pid() {
  if (g_new_data)
  {
    ...
    float target_speed = interpolate(g_speed_trajectory.target_vel_start, g_speed_trajectory.target_vel_end, TRAJECTORY_DELTA_MILLIS, delta_plan_life_ms);
    float target_acc = interpolate(g_speed_trajectory.target_acc_start, g_speed_trajectory.target_acc_end, TRAJECTORY_DELTA_MILLIS, delta_plan_life_ms);
    if (g_display_state.status_screen.throttle == MODULE_STATUS_ENABLED && g_display_state.status_screen.brakes == MODULE_STATUS_ENABLED) {
      ...
      send_control(g_long_pid.control);
    } else {
      send_control(0.0);
